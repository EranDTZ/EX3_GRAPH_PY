...
class DiGraph:
    ...
    def add_edge(self, id1: int, id2: int, weight: float) -> bool:
        self.edges[self.edgeID] = Edge(id1,id2,weight)
        self.edgeID = self.edgeID+1
        self.nodes[id1].inEdges.add(self.edges[self.edgeID])
        self.nodes[id1].outEdges.add(self.edges[self.edgeID])
        """
    ...
    def add_node(self, node_id: int, pos: tuple = None) -> bool:
        try:
            self.nodes.add(Nodes(node_id,pos))
            return True
        except:
            print("Error")
            return False
